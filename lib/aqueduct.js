...
class Aqueduct extends FilteredEventEmitter {
  ...
  runFlow(localName) {
    if(localName in this.flows) {
      this.flows.emit(SyncEvents.SYNC_START)
      return true
    }
    return false
  }
  ...
  _buildFlow(pipe, remote, local) {
    ...
    const log = loggerFactory('flow', `${pipe.remote}->${pipe.local}`)
    const findUpdated = () => {
    ...
    const upsertWithEvents = upsertEvents(local.upsert.bind(local), events)
    const upsert = upsertAdapter(pipe, remote, upsertWithEvents, saveSyncState)

    // create schedule for pipe
    const interval = pipe.interval || Aqueduct.defaults.SYNC_INTERVAL
    const runNow = pipe.runAtStartup !== false
    this.flows[pipe.local] = flow(findUpdated, upsert, interval, log, runNow)
