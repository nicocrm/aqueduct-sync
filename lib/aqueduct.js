...
class Aqueduct {
  ...
  _buildFlow(pipe) {
    ...
    const findUpdated = R.compose(remote.findUpdated, getSyncState)
    const cleanse = R.compose(pipe.cleanse, R.pick(pipe.fields))
    const upsert = upsertAdapter(local.upsert, cleanse, R.compose(saveSyncState, remote.getRevId))
    // create schedule for pipe
    flow(findUpdated, upsert, pipe.interval || Aqueduct.defaults.SYNC_INTERVAL)
  }
}
