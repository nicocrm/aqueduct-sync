...
class Aqueduct extends EventEmitter {
  ...
  start() {
    ...
    const tapEvents = tap(this.queue)

    for(let p of this.pipes) {
      this._startPipe(tapEvents, p)
  ...
  _handleTap(tapEvents, pipe, remote, local) {
    ...
    const upsert = upsertAdapter(local.upsert.bind(local), pipe.cleanse, R.identity)
    const prepare = R.compose(pipe.prepare, R.prop('data'))
    const handleCreate = R.compose(
      logPromiseError(log),
      R.composeP(upsert, remote.create.bind(remote)),
      prepare)
    tapEvents
