...
const buildJoint = require('aqueduct-pipe-joints')
  ...
  addPipe(pipeConfig) {
    this.pipes.push({
      ...
      cleanse: R.compose(
        ...
        pipeConfig.map ? propertyMapper(pipeConfig.map) : R.identity,
        R.pick(pipeConfig.fields)),
      prepare: R.compose(
        R.pick(pipeConfig.fields),
        pipeConfig.map ? propertyMapper(pipeConfig.map, true) : R.identity,
