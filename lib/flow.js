        ...
        .on('end', () => {
          promise
            .catch((err) => {
              ...
              logger.error(
                'handling error from rejected promise - SHOULD NOT HAPPEN',
                err
              )
            })
...
module.exports = function flow({
  ...
  let scheduled = false,
    running = false
  const events = new SyncEvents(new EventEmitter(), {
    local,
    source: 'remote',
  })
  ...
  const runSync = () => {
    ...
    return findUpdated()
      .then(
        ...
        (err) => {
          logger.error(
            'Error getting updated records (probably getting the sync state from the local database)',
            err
          )
        }
      )
  ...
  const reread = ({record}) => {
    // we cant use the same inside of the readRecords function because that one needs to see the errors
    upsertWithEvents(record).catch((err) => {
      logger.error('Unhandled error in re-read', err)
      // should we somehow put the record back on the queue now so we can re-read it?
      // Probably not, since this method will typically not do remote stuff so it's not super likely to
      // error, and if it does, it's probably going to keep erroring
    })
  }

  events.on(SyncEvents.SYNC_COMPLETE, scheduleSync)
  events.on(SyncEvents.SYNC_START, runSync)
  events.on(SyncEvents.REREAD, reread)
  if (runNow)
